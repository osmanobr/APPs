...
function initDB(){
    var shortName = 'BancoDB';
    var version = '1.0';
    var displayName = 'MeuBancoDB';
    var maxSize = 65536; // Em bytes
    localDB = window.openDatabase(shortName, version, displayName, maxSize);
}

function createTables(){
    var query = 'CREATE TABLE IF NOT EXISTS agenda(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, nome VARCHAR NOT NULL, idade VARCHAR NOT NULL);';
    try {
        localDB.transaction(function(transaction){
            transaction.executeSql(query, [], nullDataHandler, errorHandler);
            updateStatus("Tabela 'agenda' status: OK.");
        });
    } 
    catch (e) {
        updateStatus("Erro: Data base 'agenda' não criada " + e + ".");
        return;
    }
}
...
function onUpdate(){
    ...
    if (nome == "" || idade == "") {
        ...
        var query = "update agenda set nome=?, idade=? where id=?;";
        try {
            localDB.transaction(function(transaction){
...
function onCreate(){
    ...
    if (nome == "" || idade == "") {
        ...
        var query = "insert into agenda (nome, idade) VALUES (?, ?);";
        try {
            localDB.transaction(function(transaction){
