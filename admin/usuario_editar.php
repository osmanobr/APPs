...
$usuario_id_edit = $_GET['id'] ?? null;
if (!$usuario_id_edit) {
    $_SESSION['error_message'] = "ID do usuário não fornecido para edição.";
    redirect(APP_URL . '/admin/usuarios_listar.php');
    exit;
}

$usuarioModel = new Usuario();
$usuario_edit = $usuarioModel->getById($usuario_id_edit);

if (!$usuario_edit) {
    $_SESSION['error_message'] = "Usuário não encontrado (ID: " . escape_html($usuario_id_edit) . ").";
    redirect(APP_URL . '/admin/usuarios_listar.php');
    exit;
}

$user_name = escape_html($_SESSION['user_name'] ?? 'Usuário'); // Usuário logado
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6"><h1><?php echo $page_title; ?>: <?php echo escape_html($usuario_edit['nome']); ?></h1></div>
          <div class="col-sm-6">
        ...
        <div class="row">
          <div class="col-md-8 offset-md-2">
            <div class="card card-warning"> <!-- Card warning para edição -->
              ...
              <form id="formEditarUsuario" action="<?php echo APP_URL; ?>/controllers/UsuarioController.php" method="POST">
                <input type="hidden" name="action" value="edit">
                <input type="hidden" name="id" value="<?php echo escape_html($usuario_edit['id']); ?>">
                <div class="card-body">

                  <div class="form-group">
                    <label for="nome">Nome Completo <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" id="nome" name="nome" required value="<?php echo escape_html($usuario_edit['nome']); ?>">
                  </div>

                  <div class="form-group">
                    <label for="email">Email <span class="text-danger">*</span></label>
                    <input type="email" class="form-control" id="email" name="email" required value="<?php echo escape_html($usuario_edit['email']); ?>">
                  </div>

                  <div class="form-group">
                    <label for="senha">Nova Senha (opcional)</label>
                    <input type="password" class="form-control" id="senha" name="senha" minlength="6">
                    <small class="form-text text-muted">Deixe em branco para manter a senha atual. Mínimo de 6 caracteres se for alterar.</small>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control select2bs4" id="nivel_acesso" name="nivel_acesso" required style="width: 100%;">
                      ...
                      <?php foreach ($niveis_acesso_map as $nivel_key): ?>
                        <option value="<?php echo $nivel_key; ?>" <?php echo ($usuario_edit['nivel_acesso'] == $nivel_key) ? 'selected' : ''; ?>>
