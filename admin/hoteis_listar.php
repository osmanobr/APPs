...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    <ul class="navbar-nav">
      ...
      <li class="nav-item d-none d-sm-inline-block"><a href="<?php echo APP_URL; ?>/admin/hoteis_listar.php" class="nav-link active">Hotéis</a></li>
    </ul>
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open"> <!-- Manter aberto o menu de hoteis -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Lista de todos os hotéis cadastrados</h3>
                <div class="card-tools">
                  <a href="<?php echo APP_URL; ?>/admin/hotel_criar.php" class="btn btn-success"><i class="fas fa-plus"></i> Novo Hotel</a>
                </div>
              </div>
              <div class="card-body">
                <?php if (empty($hoteis)): ?>
                  <div class="alert alert-info">Nenhum hotel cadastrado ainda.</div>
                <?php else: ?>
                  <table id="hoteisTable" class="table table-bordered table-hover">
                    <thead><tr><th>Nome do Hotel</th><th>Endereço</th><th>Ações</th></tr></thead>
                    <tbody>
                    <?php foreach ($hoteis as $hotel): ?>
                    <tr>
                      <td><?php echo escape_html($hotel['nome']); ?></td>
                      <td><?php echo escape_html($hotel['endereco'] ?? 'N/A'); ?></td>
                      <td>
                        <a href="<?php echo APP_URL; ?>/admin/hotel_editar.php?id=<?php echo $hotel['id']; ?>" class="btn btn-sm btn-info"><i class="fas fa-edit"></i> Editar</a>
                        <button class="btn btn-sm btn-danger btn-delete-hotel" data-id="<?php echo $hotel['id']; ?>" data-nome="<?php echo escape_html($hotel['nome']); ?>"><i class="fas fa-trash"></i> Excluir</button>
                      </td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
  ...
  <!-- Modal de Confirmação de Exclusão -->
    <div class="modal fade" id="deleteHotelModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Tem certeza que deseja excluir o hotel "<span id="hotelNomeModal"></span>"?
                    <br><small class="text-danger">Atenção: Se houver apartamentos ou outras dependências associadas a este hotel, a exclusão pode ser impedida.</small>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                    <form id="deleteHotelForm" action="<?php echo APP_URL; ?>/controllers/HotelController.php" method="POST" style="display: inline;">
...
$(document).ready(function() {
    $('#hoteisTable').DataTable({"language": {"url": "//cdn.datatables.net/plug-ins/1.11.5/i18n/pt-BR.json"},"order": [[0, "asc"]]});
    $('.btn-delete-hotel').on('click', function() {
        var hotelId = $(this).data('id');
        var hotelNome = $(this).data('nome');
        $('#hotelIdToDelete').val(hotelId);
        $('#hotelNomeModal').text(hotelNome);
